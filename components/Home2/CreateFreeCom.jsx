...
console.log("picked file:", file);
  ...
  // only append if values exist (avoid undefined)
  if (credentials?.description) form.append('description', credentials.description);
  if (credentials?.medName) form.append('medName', credentials.medName);
  if (user?._id) form.append('idUser', user._id);
  ...
  for (const [k, v] of form.entries()) {
    if (v instanceof File) {
      console.log('Form:', k, '=> File { name:', v.name, ', size:', v.size, ', type:', v.type, '}');
    } else {
      console.log('Form:', k, '=>', v);
    }
  }
  // Also produce a quick readable object (files will appear as File objects)
  try {
    const simple = {};
    for (const [k, v] of form.entries()) simple[k] = v instanceof File ? v.name : v;
    console.log('Form snapshot:', simple);
  } catch (err) {
    console.warn('Could not snapshot FormData', err);
  }
